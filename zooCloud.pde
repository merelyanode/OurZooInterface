...
Sphere outerSphere; // the main actor, container for all the little particles
Sphere middleSphere;
Sphere innerSphere;
...
//Xe defining the coordinates for the Sphere container
  ...
  outerSphere = new Sphere(xPos, yPos, zPos, radius * 3/3, 4);
  middleSphere = new Sphere(xPos, yPos, zPos, radius * 2/3, 3);
  innerSphere = new Sphere(xPos, yPos, zPos, radius * 1/3, 2);
  //Xe create a number of sphereItems that is equal to the  value of the var, "num".
  for (int i = 0; i < mostPopularWords.size(); i++) {
    outerSphere.addSphereItem(mostPopularWords.get(i));
  }
  
  for (int i = 0; i < mostPopularPhrases.size(); i++) {
    middleSphere.addSphereItem(mostPopularPhrases.get(i));
  }
  
  for (int i = 0; i < allEntries.size(); i++) {
    innerSphere.addSphereItem(allEntries.get(i));
  }
...
// Uncomment the next line to visualise the control point as a black circle
  ...
  outerSphere.render( cp.getX(), cp.getY() );
  middleSphere.render( cp.getX(), cp.getY() );
  innerSphere.render( cp.getX(), cp.getY() );
  
  Object highestUp = null;
  
  for (int i = 0; i < outerSphere.items.size(); i++) {
