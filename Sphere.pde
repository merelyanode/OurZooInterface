...
//Xe Is this just a new Sphere class object that happens to be named "Sphere"?
  Sphere(float xPos, float yPos, float zPos, float radius, int type) { 
    ...
    this.sphereMode = type;
    icon = loadImage("icon.png");
    
    lowU = random(0, 1 - groupRange);
    lowV = random(0, 1 - groupRange);
  }
  ...
  public void addSphereItem(Object o) {
    ...
    float u = random(lowU, lowU + groupRange);
    float v = random(lowV, lowV + groupRange);
    ...
    si.phi = acos(2 * v - 1);
    // set size
    si.itemSize = sphereMode * 4;
    
    if (sphereMode == 2) {
       si.clr = color(255, 0, 0);
    }
    
    if (sphereMode == 3) {
       si.clr = color(0, 255, 0);
    }
    
    if (sphereMode == 4) {
       si.clr = color(0, 0, 255);
    }
