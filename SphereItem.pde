...
class SphereItem {
  ...
  PopularPhrase popularPhrase;
  Word word;

  public void SphereItem(Object item) {
    
    if (item instanceof PopularWord) {
      type = 1;
      popularWord = (PopularWord) item;
    }
    
    if (item instanceof PopularPhrase) {
      type = 2;
      popularWord = (PopularPhrase) item;
    }
    
    if (item instanceof Word) {
      type = 3;
      popularWord = (Word) item;
    }
